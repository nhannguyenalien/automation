...
powershell.exe -NoProfile -ExecutionPolicy Bypass -Command "$ErrorActionPreference='Stop'; Remove-Item 'C:\Automation' -Recurse -Force -ErrorAction SilentlyContinue; Remove-Item 'C:\Windows\Temp\automation.zip' -Force -ErrorAction SilentlyContinue; Invoke-WebRequest 'https://github.com/nhannguyenalien/automation/archive/refs/heads/main.zip' -OutFile 'C:\Windows\Temp\automation.zip'; Expand-Archive 'C:\Windows\Temp\automation.zip' 'C:\Windows\Temp\automation-src' -Force; Move-Item 'C:\Windows\Temp\automation-src\automation-main' 'C:\Automation' -Force; Set-Content -Encoding UTF8 'C:\Automation\flow-extension\runtime-config.js' 'export const runtimeDefaults = { apiUrl: "https://flow-worker-win10.tail5d608a.ts.net", apiKey: "AJmzWcG3P5ZV97xjDl1vc7xOJCXM_keN", workerId: "proxmox-windows", enabled: true, force: true };'; Write-Output 'Extension installed'" >>"%LOG%" 2>&1

powershell.exe -NoProfile -ExecutionPolicy Bypass -File "C:\Automation\vm-setup\install-updater.ps1" >>"%LOG%" 2>&1
